"""Iterative NQS + DCI+PT2 seed -> Krylov classical expansion.

Pipeline:
  Phase 1: Generate DCI basis (HF + singles + doubles), then expand via
           PT2-style H-connection growth (expand_basis_via_connections).
           Diagonalise to get seed energy.
  Phase 2: Run iterative NQS + SKQD (Krylov expansion via H-connections).
  Phase 3: Report best energy across DCI+PT2 seed and iterative NQS+SKQD.
...
capturing perturbative corrections beyond CISD.
"""
...
def main() -> None:
    ...
    # --- Results summary ---
    print("\n" + "=" * 60)
    print("ITERATIVE NQS + DCI+PT2 -> KRYLOV CLASSICAL RESULTS")
    print("=" * 60)
    print(f"DCI+PT2 energy   : {dci_energy:.10f} Ha  (basis: {dci_pt2_basis.shape[0]})")
    print(f"NQS+SKQD energy  : {nqs_result.energy:.10f} Ha  (basis: {nqs_result.diag_dim})")
