"""Iterative NQS + DCI seed -> Quantum Circuit Krylov.

Pipeline:
  Phase 1: Generate DCI basis (HF + singles + doubles) deterministically,
           diagonalise to get seed energy.
  Phase 2: Iterative NQS warmup via SQD (few iterations) to improve
...
def main() -> None:
    ...
    # --- Results summary ---
    print("\n" + "=" * 60)
    print("ITERATIVE NQS + DCI -> QUANTUM KRYLOV RESULTS")
    print("=" * 60)
    print(f"DCI seed energy  : {dci_energy:.10f} Ha  ({(dci_energy - exact_energy) * 1000:.4f} mHa)")
